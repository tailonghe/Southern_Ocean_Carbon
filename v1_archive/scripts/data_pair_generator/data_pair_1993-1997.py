...
def clim_chl_file(dt, yrep):
    chllist = glob.glob('../SOCO2_phase2/CHL/8day/L3m_%04d*.nc_1x1' % yrep)
...
for yy in range(1993, 1998):
    ...
    while tnow < tend and curr <= 72:  
        for tc in range(5):
            ...
            if fnow == None:
                fnow = clim_chl_file(subt, 1997)

            if fnow == None:
                fnow = clim_chl_file(subt, 1998)

            if fnow == None:
                fnow = clim_chl_file(subt, 1999)

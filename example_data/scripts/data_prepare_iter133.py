...
fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_DIC_1x1.nc')
...
etimes = datetime.datetime(2012, 12, 1, 0, 0, 0) + datetime.timedelta(seconds=1)*times
stimes = etimes - datetime.timedelta(days=5)
print(stimes.shape, stimes[0])
...
# for i in range(5):
	...
	fh.close()
	fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_Salt_1x1.nc')
	salt = fh.variables[ 'SALT' ][i, :48, :, :]
	fh.close()
	fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_Theta_1x1.nc')
	...
	fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_Chl_1x1.nc')
	chl = fh.variables['BLGCHL'][i, 0, :, :]
	fh.close()

	fh = nc.Dataset('iter133/bsose_i133_2013to2018_1dy_SSH_1x1.nc')
	ssha = np.nanmean( fh.variables['ETAN'][i*5:(i+1)*5, :, :], axis=0 )
	fh.close()

	fh = nc.Dataset('iter133/bsose_i133_2013to2018_1dy_pCO2_1x1.nc')
	pco2 = np.nanmean( fh.variables['BLGPCO2'][i*5:(i+1)*5, :, :], axis=0 )
	fh.close()

	fh = nc.Dataset('iter133/bsose_i133_2013to2018_1dy_surfTflx_1x1.nc')
	tflux = np.nanmean( fh.variables['TFLUX'][i*5:(i+1)*5, :, :], axis=0 )
	fh.close()


	fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_Uvel_1x1.nc')
	uvel = fh.variables['UVEL'][i, 0, :, :]
	fh.close()

	fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_Vvel_1x1.nc')
	vvel = fh.variables['VVEL'][i, 0, :, :]
	fh.close()

	fh = nc.Dataset('iter133/bsose_i133_2013to2018_5day_Wvel_1x1.nc')
	...
	np.savez('X/X_%03d.npz'%(i+609), lat=llats, lon=llons, latb=llatb, lonb=llonb, x=xnow)
	np.savez('Y/Y_%03d.npz'%(i+609), lat=llats, lon=llons, latb=llatb, lonb=llonb, y=ynow)